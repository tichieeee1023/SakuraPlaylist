...
// 게시글 목록 최신순으로 가져옴
export const fetchguestbooksFromFirestore = async () =>{ 
    const guestbooksQuery = query(guestbooksRef, orderBy('createdAt', 'desc'))
    const snapshot = await getDocs(guestbooksQuery) 
...
export const addPostToFirestore = async (post) => { 
    const docRef = await addDoc(guestbooksRef, {
        ...post,
...
export const deletePostFromFirestore = async (id) => {
    const postDocRef = doc(db, 'guestbooks', id)
    await deleteDoc(postDocRef)
}

// 게시글 수정
export const updatePostInFirestore = async (updatedPost) => {
    const postDocRef = doc(db, 'guestbooks', updatedPost.id)
